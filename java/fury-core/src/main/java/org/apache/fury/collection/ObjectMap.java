...
public class ObjectMap<K, V> implements Map<K, V> {
	...
	/**
	 * Constructs an empty {@code ObjectMap} with the default initial capacity (16)
	...
	/**
	 * Constructs an empty {@code ObjectMap} with the specified initial capacity
	 * and the default load factor of {@code 0.45}.
	...
	/**
	 * Constructs an empty {@code ObjectMap} with the specified load factor and an initial
	 * capacity of 16.
	 * <p>
	 ...
	 */
	public ObjectMap(float loadFactor) {
		this(DEFAULT_START_SIZE, loadFactor);
	}

	/**
	 * Constructs an empty {@code ObjectMap} with the specified load factor and initial
	 * capacity.
	 * <p>
	 * The load factor will cause the map to double in size when the number
	 * of items it contains has filled up more than {@code loadFactor} of the available
	...
	public ObjectMap(ObjectMap<? extends K, ? extends V> other) {
		size = other.size;
		mask = other.mask;
		shift = other.shift;
	...
	public V put (K key, @Nullable V value) {
		if(key == null) throw new NullPointerException("ObjectMap does not permit null keys.");
	...
	public static class EntrySet<K, V> extends AbstractSet<Map.Entry<K, V>> {
		...
		public EntrySet(ObjectMap<K, V> map) {
			this.map = map;
		}
		/**
	...
	public static class EntryIterator<K, V> implements Iterable<Map.Entry<K, V>>, Iterator<Map.Entry<K, V>> {
		...
		public EntryIterator(ObjectMap<K, V> map) {
			this.map = map;
			entry = new Entry<>();
			reset();
		}
	...
	public static class KeySet<K> extends AbstractSet<K> {
		protected final ObjectMap<K, ?> map;
		public KeySet(ObjectMap<K, ?> map) {
			this.map = map;
		}
	...
	public static class ValueCollection<V> extends AbstractCollection<V> {
		protected final ObjectMap<?, V> map;
		public ValueCollection(ObjectMap<?, V> map) {
			this.map = map;
		}
	...
	protected static class KeyIterator<K> implements Iterator<K>, Iterable<K> {
		...
		public KeyIterator(ObjectMap<K, ?> map) {
	...
	protected static class ValueIterator<V> implements Iterator<V>, Iterable<V> {
		...
		public ValueIterator(ObjectMap<?, V> map) {
			iter = map.entrySet().iterator();
		}

 ...
 * safety goes a long way. Other cuckoo hashing implementations use mechanisms such as a "stash" for
 * frequently-colliding keys; this may work to some extent for a fixed-size stash, but it can fail badly
 * for a resizable stash if too many keys collide over all bits. The theoretical background behind a
 * stash also seems to be based on a different model for how hash code generation works (not the way Java
...
public class IdentityObjectIntMap<K> {
	...
	private transient MapStatistics stat;
	...
	public int profilingPutOrGet(K key, int value) {
		...
		if(flipThreshold == 0)
			return profilingPutOrGetLinear(key, value);
	...
	public int profilingPutOrGetLinear(K key, int value) {
		K[] keyTable = this.keyTable;
		int mask = this.mask;
		for (int i = (int)(System.identityHashCode(key) * hashMultiplier1 >>> shift);
				;
				i = i + 1 & mask) {
	...
	protected int profilingPutOrGetFallback (K key, int value, int hc) {
		int loop = 0;
		while (loop++ < flipThreshold) {
			int hr1 = (int)(hashMultiplier1 * hc >>> shift) | 1;
			K k1 = keyTable[hr1];
	...
	public Map<K, Integer> toIdentityHashMap() {
		Map<K, Integer> map = new IdentityHashMap<>(size);
		K[] keyTable = this.keyTable;
		int[] valueTable = this.valueTable;
		for (int i = 0, n = keyTable.length; i < n; i++) {
			K k = keyTable[i];
	...
	/**
	 * Of the three toFooBarMap methods, this is recommended
	 * because it allows keeping the exact structure of an IdentityObjectIntMap
	 * and only changing the value type.
	 * @return an IdentityMap (from this package) with the same key references and boxed values
	 */
	public Map<K, Integer> toIdentityMap() {
		IdentityMap<K, Integer> map = new IdentityMap<>(1, loadFactor);
		map.size = size;
		map.mask = mask;
		map.shift = shift;
